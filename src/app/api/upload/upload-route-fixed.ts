...
import { uploadBuffer, uploadJson, sourceFileKey, extractionKey } from "@/lib/r2"
...
export async function POST(req: NextRequest) {
  ...
  // Script sends a "meta" JSON string + "sourceFile" + "extractionJson"
  let docId: string | null = null
  let programCode: string | null = null
  let courseCode: string | null = null
  let workflowRaw: string | null = null
  let tenantSlug = "wgu" // default — push script has no --tenant flag
  ...
  // Accept both naming conventions for files
  const sourceFile =
  ...
  const srcKey  = sourceFileKey(tenant.id, review.id, ext)
  const extrKey = extractionKey(tenant.id, review.id)

  await uploadBuffer(srcKey, sourceBuffer, sourceFile.type || "application/octet-stream")
